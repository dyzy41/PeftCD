...
#     csv_filename = os.path.join(os.path.dirname(args.pred), 'overall_result.csv')
...
def dir_metric(pred_path, gt_path, pred_suffix, gt_suffix, n_class):
    ...
    metrics = CM2Metric(confu_mat_total)
    save_metrics(os.path.join(os.path.dirname(pred_path), 'overall.csv'), metrics)


def save_metrics(save_path, metrics):
    # 只挑我们关心的几个指标
    paper_metrics = [
        metrics[0],        # OverallAccuracy
        metrics[4][1],     # ClassIoU of 类别1
        metrics[5][1],     # ClassF1  of 类别1
        metrics[6][1],     # ClassRecall of 类别1
        metrics[7][1],     # ClassPrecision of 类别1
        metrics[1],        # MeanF1
        metrics[2],        # MeanIoU
    ]

    # 这里改用 np.isscalar 来判断“是不是单个数值”
    paper_metrics = [
        round(float(v) * 100, 2) if np.isscalar(v)
        else [round(float(x) * 100, 2) for x in v]
        for v in paper_metrics
    ]

    # 写入 CSV
    names = 'OverallAccuracy,ClassIoU,ClassF1,ClassRecall,ClassPrecision,MeanF1,MeanIoU'.split(',')
    for i in range(len(names)):
        print(names[i]+': '+str(paper_metrics[i]))
    with open(save_path, 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(names)
        writer.writerow(paper_metrics)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pred', type=str, default='/home/jic2/dsj_code/daily_code/24/rscd/test_result')
    parser.add_argument('--label', type=str, default='/media/jic2/HDD/DSJJ/CDdata/CLCD/test/label')
    parser.add_argument('--pred_suffix', type=str, default='.png')
    parser.add_argument('--label_suffix', type=str, default='.png')
    args = parser.parse_args()

    x = dir_metric(args.pred, args.label, args.pred_suffix, args.label_suffix, 2)
